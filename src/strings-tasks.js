...
function removeLastOccurrences(str, value) {
  const lastIndexOf = str.lastIndexOf(value);
  if (lastIndexOf !== -1)
    return str.slice(0, lastIndexOf) + str.slice(lastIndexOf + value.length);
  return str;
}
...
function findLongestWord(sentence) {
  ...
  sentence.split(' ').forEach((el) => {
    if (longest.length < el.length) longest = el;
  });
  return longest;
}
      ...
      return word
        ...
        .map((chart) => {
          if (chart !== chart.toLowerCase()) {
            return chart.toLowerCase();
          }
          return chart.toUpperCase();
        })
        .join('');
    })
    .join(' ');
...
function extractEmails(str) {
  const emailArray = [];
  str.split(';').forEach((el) => emailArray.push(el));
  return emailArray;
}
...
function getCardId(value) {
  ...
  return playingCards.findIndex((elem) => elem === value);
}
